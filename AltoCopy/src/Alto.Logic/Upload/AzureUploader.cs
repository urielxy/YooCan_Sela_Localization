...
namespace Alto.Logic.Upload
{
    public class AzureUploader : BlobUploader
    {
        ...
        public AzureUploader(CloudStorageAccount storageAccount, IImageLogic imageLogic, IServiceProvider serviceProvider)
            : base(imageLogic, serviceProvider)
        {
            _storageAccount = storageAccount;
        }

        public override async Task<string> UploadStreamAsync(Stream stream, string containerName, string fileName,
            int? width = null, int? height = null, string maxAge = "max-age=31536000", int quality = 90)
        {
            //TODO fix resize / make it optional
            //using (var resized = Resize(stream, width, height, quality))
            {
